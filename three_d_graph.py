...
from graphclass import SongGraph


def convert_range_string_to_tuple(range_string: str) -> tuple:
    """ Converts a range string into a tuple.
    """
...
sample_song_attributes = {
    'Song1': {'valence': (0.5, 0.7), 'energy': (0.6, 0.8), 'danceability': (0.4, 0.6), 'loudness': (-8, -6),
              'instrumentalness': (0.1, 0.3), 'tempo': (100, 120), 'speechiness': (0.1, 0.3), 'artist': 'Artist1'},
    'Song2': {'valence': (0.7, 0.9), 'energy': (0.5, 0.7), 'danceability': (0.6, 0.8), 'loudness': (-10, -8),
              'instrumentalness': (0.0, 0.2), 'tempo': (120, 140), 'speechiness': (0.3, 0.5), 'artist': 'Artist2'},
    'Song3': {'valence': (0.6, 0.8), 'energy': (0.7, 0.9), 'danceability': (0.5, 0.7), 'loudness': (-6, -4),
              'instrumentalness': (0.1, 0.3), 'tempo': (140, 160), 'speechiness': (0.2, 0.4), 'artist': 'Artist3'},
    'Song4': {'valence': (0.4, 0.6), 'energy': (0.8, 1.0), 'danceability': (0.7, 0.9), 'loudness': (-7, -5),
              'instrumentalness': (0.2, 0.4), 'tempo': (110, 130), 'speechiness': (0.2, 0.4), 'artist': 'Artist4'},
    'Song5': {'valence': (0.5, 0.7), 'energy': (0.6, 0.8), 'danceability': (0.5, 0.7), 'loudness': (-8, -6),
              'instrumentalness': (0.1, 0.3), 'tempo': (100, 120), 'speechiness': (0.1, 0.3), 'artist': 'Artist5'}
}
...
def create_3d_scatter(x_attribute: str, y_attribute: str, z_attribute: str) -> None:
    ...
    fig = go.Figure(data=[go.Scatter3d(
        ...
        hoverinfo='text',
        marker={"size": 1, "opacity": 0.8}
    )])

    # Set labels and title
    fig.update_layout(
        scene={"xaxis_title": x_attribute.capitalize(), "yaxis_title": y_attribute.capitalize(),
               "zaxis_title": z_attribute.capitalize()},
        title=f'{x_attribute.capitalize()}, {y_attribute.capitalize()}, and {z_attribute.capitalize()}'
              f' Ranges Across Songs',
        hovermode='closest'
    )
...
selected_attributes = input(
    "Select three attributes separated by commas from the given options (valence, energy, danceability, loudness, "
    "instrumentalness, tempo, speechiness, artist): ").split(
    ',')
...
if __name__ == '__main__':
    python_ta.check_all(config={
        'extra-imports': ['plotly.graph_objs', 'graphclass'],
        'allowed-io': ['convert_range_string_to_tuple'],
        'max-line-length': 120
    })
